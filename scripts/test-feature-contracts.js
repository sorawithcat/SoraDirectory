...
const formatToolbar = read('js/formatToolbar.js');
...
const methodWorkbench = read('js/methodWorkbench.js');
...
assert.ok(index.includes('window.SORA_ASSET_VERSION = Date.now().toString(36)'), 'automatic asset cache busting missing');
assert.ok(index.includes("loadBootstrapScript(versionScript('js/scriptLoader.js'))"), 'script loader must use the same dynamic asset version');
assert.ok(index.includes('css/design-system.css?v=${window.SORA_ASSET_VERSION}'), 'design-system stylesheet must use the dynamic asset version');
assert.ok(index.includes('content="no-cache, no-store, must-revalidate"'), 'editor HTML cache policy missing');
assert.equal(index.includes('<script src="js/scriptLoader.js'), false, 'script loader still uses a fixed cache version');
...
assert.ok(workspace.includes("overlay.classList.add('above-custom-dialog')"), 'nested feature dialog layer contract missing');
assert.ok(workspace.includes("activeCustomDialog.setAttribute('inert', '')"), 'covered custom dialog must suspend interaction');
assert.ok(workspace.includes("covered.element.removeAttribute('inert')"), 'covered custom dialog interaction must be restored');
assert.ok(dialog.includes('function activateCustomDialog()'), 'custom dialog focus lifecycle missing');
assert.ok(dialog.includes("toast.setAttribute('role', type === 'error' ? 'alert' : 'status')"), 'toast announcements missing');
...
assert.ok(formatToolbar.includes('colorPickerDialog(currentColor'), 'text color action missing');
assert.ok(formatToolbar.includes("coloredSelection.querySelectorAll('a')"), 'selected links must receive text color directly');
assert.ok(formatToolbar.includes("window.open(href, '_blank', 'noopener,noreferrer')"), 'editor external links must isolate the opener');
...
assert.ok(methods.includes("customDialogOverlay.setAttribute('aria-hidden', 'false')"), 'method dialog visible state semantics missing');
assert.ok(methods.includes("customDialogOverlay.setAttribute('aria-hidden', 'true')"), 'method dialog hidden state semantics missing');
assert.equal(methods.includes('methodTestBtn'), false, 'method configuration still exposes the removed test action');
assert.equal(methodWorkbench.includes('function test('), false, 'method sandbox implementation was not removed');
assert.equal(methodWorkbench.includes('data-action-name="test"'), false, 'method manager still exposes the removed test action');
assert.equal(methodWorkbench.includes('方法沙盒测试'), false, 'method sandbox dialog remains in product code');
...
assert.ok(index.includes('z-index: 13000') && index.includes('z-index: 14000'), 'nested dialog and toast layers must stay above feature dialogs');
assert.ok(index.includes('id="toastContainer" aria-live="polite"'), 'toast live region missing');
...
assert.ok(methodWorkbench.includes('core.flattenFlow(config)'), 'method flow view does not use the shared core');
...
assert.ok(reusableBlocks.includes("document.querySelector('.markdown-preview')"), 'reusable block insertion must resolve the actual editor root');
assert.equal(reusableBlocks.includes('window.markdownPreview'), false, 'reusable block insertion must not depend on a missing window property');
assert.ok(reusableBlocks.includes('dataset.blockError') && reusableBlocks.includes('当前没有其他目录可供引用'), 'reusable block empty and error feedback missing');
...
assert.ok(extensionPacks.includes('未声明 component-presets 能力'), 'extension pack component capability declaration is not enforced');
assert.ok(extensionPacks.includes('function getComponentPresets()'), 'installed component presets are not exposed to the component library');
assert.ok(extensionPacks.includes('浏览器拒绝写入本地存储，扩展包未导入') && extensionPacks.includes('resultBox.textContent = describe(result)'), 'extension-pack persistence failures are not shown in the dialog');
...
assert.ok(componentLibrary.includes('getComponentPresets?.()') && componentLibrary.includes('data-preset'), 'extension-pack component presets are not selectable');
assert.ok(componentLibrary.includes("document.querySelector('.markdown-preview')"), 'component insertion must resolve the actual editor root');
assert.equal(componentLibrary.includes('window.markdownPreview'), false, 'component insertion must not depend on a missing window property');
assert.ok(componentLibrary.includes('data-component-error') && componentLibrary.includes('后锚点必须位于前锚点之后'), 'component insertion validation must remain visible in the dialog');
